...
export default function LandingPage() {
  ...
  return (
    <div className="flex flex-col gap-12 py-4">
      {/* Hero Section */}
      <section className="bg-emerald-950 text-white rounded-3xl p-8 md:p-12 shadow-xl relative overflow-hidden flex flex-col gap-6">
        <div className="absolute top-0 right-0 p-8 opacity-5">
          <Landmark className="h-64 w-64" />
        </div>

        <div className="flex flex-col gap-2 max-w-3xl z-10">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-900/50 text-emerald-300 border border-emerald-800 px-3 py-1 text-xs font-bold leading-none w-max">
            <BookOpen className="h-3.5 w-3.5" />
            Tugas Akhir Big Data & Cloud Computing
          </span>
          <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight text-emerald-50 leading-tight">
            Dashboard Analitik Harga Pangan Indonesia
          </h1>
          <p className="text-emerald-200/90 text-sm md:text-lg leading-relaxed mt-2 font-semibold">
        ...
        <div className="flex flex-col sm:flex-row gap-3 z-10 shrink-0 mt-2">
          ...
          <Link href="/dataset" passHref>
            <Button className="bg-emerald-900 text-emerald-100 hover:bg-emerald-850 w-full sm:w-auto font-semibold">
      ...
      <section className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {steps.map((step, idx) => {
            ...
            return (
              <Card key={idx} className="flex flex-col gap-4 bg-white border border-slate-200 hoverEffect">
                <div className={`${step.color} rounded-2xl p-3.5 shrink-0 w-max`}>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center bg-white border border-slate-200 rounded-3xl p-8 shadow-xs">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-6 shrink-0 mt-2 font-bold text-slate-700">
            <div className="flex flex-col">
              <span className="text-2xl font-extrabold text-emerald-600">8 Komoditas</span>
              <span className="text-[10px] text-slate-400 uppercase tracking-widest">Komoditas Pokok</span>
            </div>
            <span className="h-8 w-px bg-slate-200" />
            <div className="flex flex-col">
              <span className="text-2xl font-extrabold text-emerald-600">8 Provinsi</span>
              <span className="text-[10px] text-slate-400 uppercase tracking-widest">Wilayah Studi</span>
            </div>
            <span className="h-8 w-px bg-slate-200" />
            <div className="flex flex-col">
              <span className="text-2xl font-extrabold text-emerald-600">94.2% R²</span>
              <span className="text-[10px] text-slate-400 uppercase tracking-widest">Akurasi Regresi</span>
            </div>
          </div>
        </div>

        <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200/60 flex flex-col gap-4">
          <h4 className="text-xs font-extrabold text-slate-500 uppercase tracking-widest leading-none">
